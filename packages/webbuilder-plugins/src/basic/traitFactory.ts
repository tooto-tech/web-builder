...
export type WebBuilderTraitInput =
  | string
  | (Omit<Partial<TraitProperties>, 'options'> & {
      options?: readonly WebBuilderTraitOption[]
      ui?: Record<string, unknown>
      [key: string]: unknown
    })

export const defineWebBuilderTraits = <T extends readonly WebBuilderTraitInput[]>(traits: T): T =>
  traits

export const defineGrapesTraits = (
  traits: readonly WebBuilderTraitInput[]
): (string | Partial<TraitProperties>)[] =>
  traits as unknown as (string | Partial<TraitProperties>)[]
...
export function makeSelectTrait(
  ...
  return {
    ...
    name,
    changeProp: true,
    options: selectOptions as unknown as TraitProperties['options'],
  }
}
