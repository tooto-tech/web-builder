...
const LEGACY_NUMBER_TEXT_STYLE_PROPS = [
  'font-size',
  'font-family',
  'font-weight',
  'font-style',
  ...
  domComponents.addType(WB_COUNT_UP_TYPE, {
    ...
    model: {
      ...
      init(this: any) {
        ...
        this._migrateLegacyNumberTextStyle()
        this.on(
      ...
      _migrateLegacyNumberTextStyle(this: any) {
        const children = this.components?.()?.models ?? []
        const numberText = children.find((child: any) => {
          const attrClass = `${child.getAttributes?.()?.class ?? ''}`
          const classes = typeof child.getClasses === 'function'
            ? child.getClasses().map((cls: any) => cls?.get?.('name') ?? cls?.name ?? cls?.getName?.() ?? '')
            : []
          return [attrClass, ...classes].join(' ').split(/\s+/).includes(COUNT_UP_NUMBER_CLASS)
        })
        const legacyStyle = numberText?.getStyle?.() ?? numberText?.get?.('style') ?? {}
        const migratedStyle = LEGACY_NUMBER_TEXT_STYLE_PROPS.reduce<Record<string, any>>((style, prop) => {
          const value = legacyStyle[prop]
