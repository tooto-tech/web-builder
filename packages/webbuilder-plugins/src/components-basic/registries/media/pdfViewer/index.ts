...
import { makeImagePickerTrait, makeNumberTrait, makeTextareaTrait, makeTextTrait } from '../../../traitFactory.js'
...
const script = function (this: HTMLElement) {
  ...
  const viewerNode = viewerEl
  const canvasNode = canvas
  const placeholderNode = placeholder
  const loadingNode = loadingEl
  const pageInfoNode = pageInfo
  const scaleInfoNode = scaleInfo
  ...
  function updatePlaceholder() {
    ...
    placeholderNode.innerHTML = '<p>' + escapeHtml(text) + '</p><p><code>?url=/path/to/document.pdf</code></p><p class="wb-pdf-viewer__placeholder-hint">' + escapeHtml(hint) + '</p>'
  }

  function updateButtons() {
    const hasPdf = !!pdfDoc
    prevButton.disabled = !hasPdf || pageNum <= 1
    nextButton.disabled = !hasPdf || pageNum >= pageCount
    fitButton.disabled = !hasPdf
    zoomOutButton.disabled = !hasPdf || userZoom <= 0.5
    zoomInButton.disabled = !hasPdf || userZoom >= 3
  }
  ...
  function setLoading(loading: boolean, message?: string) {
    ...
    if (loading && message) {
      const text = loadingNode.querySelector('.wb-pdf-viewer__loading-text') as HTMLElement | null
  ...
  function showEmpty() {
    ...
    setLoading(false)
    pageInfoNode.textContent = 'No PDF loaded'
    scaleInfoNode.textContent = '100%'
    updateButtons()
  }
  ...
  async function renderPage(num: number) {
    ...
    try {
      ...
      const availW = Math.max(160, viewerNode.clientWidth - horizontalPad)
      const availH = Math.max(160, viewerNode.clientHeight - verticalPad)
      ...
      canvasNode.width = Math.floor(viewport.width * outputScale)
      canvasNode.height = Math.floor(viewport.height * outputScale)
      ...
      pageInfoNode.textContent = 'Page ' + pageNum + ' / ' + pageCount
      scaleInfoNode.textContent = Math.round(userZoom * 100) + '%'
      updateButtons()
    } catch (error) {
      console.error('[WebBuilder] PDF render failed', error)
      pageInfoNode.textContent = 'Failed to render PDF'
    } finally {
      isRendering = false
  ...
  async function reloadPdf() {
    ...
    activeUrl = nextUrl
    pageInfoNode.textContent = 'Loading PDF...'
    ...
    try {
      ...
      pageInfoNode.textContent = 'Failed to load PDF'
      setLoading(false)
      placeholderNode.hidden = false
      canvasNode.hidden = true
      updateButtons()
