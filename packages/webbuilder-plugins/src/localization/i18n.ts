...
const INTERNAL_VISIBLE_CONFIG_KEYS = new Set(['fmMenuTitle'])
...
const GENERATED_CHILD_TRANSLATION_OWNER_TYPES = new Set(['wb-image', 'wb-inquiry-form'])

type LayoutSlotKey = 'header' | 'footer'

const WB_LAYOUT_PAGE_CUSTOM_KEY = 'wbLayoutSlot'
const WB_LAYOUT_PAGE_ID_CUSTOM_KEY = 'wbLayoutPageId'

const getPageLayoutSlot = (page: any): LayoutSlotKey | null => {
...
const getClassNames = (attrs: Record<string, any> | null | undefined): string[] =>
  `${attrs?.class ?? ''}`.split(/\s+/).filter(Boolean)

const isCmsDynamicContentAttrs = (attrs: Record<string, any> | null | undefined): boolean => {
  if (!attrs || typeof attrs !== 'object') return false
  return (
    Boolean(normalizeText(attrs['data-cms-html'])) ||
    Boolean(normalizeText(attrs['data-cms-repeat'])) ||
    getClassNames(attrs).includes('wb-footer-menu__list')
  )
}
...
const isTextElementComponent = (component: any): boolean =>
  getComponentType(component).toLowerCase() === 'text'
...
const isI18nSkippedAttrs = (attrs: Record<string, any> | null | undefined): boolean => {
  ...
  return (
    ...
    getClassNames(attrs).includes('notranslate') ||
    isCmsDynamicContentAttrs(attrs)
  )
}
...
const getDirectTextFieldFromProject = (
  component: any,
  source: string
): WebBuilderI18nField => {
...
const isAltLikeField = (field: WebBuilderI18nField): boolean => {
  const normalized = `${field}`.replace(/^prop:/, '')
  return field === 'alt' || /(?:^|\/)(?:alt|imageAlt)$/i.test(normalized)
}

const isNonTranslatableEntry = (
  entry: Pick<WebBuilderI18nEntry, 'field'> & { source?: unknown }
): boolean => {
  const source = normalizeText(entry.source)
  if (!source) return false
  if (isAltLikeField(entry.field) && source.toLocaleLowerCase() === 'logo') return true
  return false
}
...
const collectVisibleConfigValues = (
  ...
  Object.entries(value).some(([key, childValue]) => {
    ...
    if (INTERNAL_VISIBLE_CONFIG_KEYS.has(key)) return
...
export const ensureModelI18nKeys = (
  editor: any,
  options: EnsureModelI18nKeysOptions = {}
): string[] => {
  ...
  if (pages.length) {
    pages.forEach((page, pageIndex) => {
      if (!options.includeLayoutPages && getPageLayoutSlot(page)) return
...
const pushEntry = (
  ...
  if (isNonTranslatableEntry({ ...entry, source })) return
...
export const extractI18nEntriesFromProjectData = (
  projectData: Record<string, any> | null,
  options: ExtractI18nEntriesOptions = {}
) => {
...
const hasClassName = (component: any, className: string): boolean =>
  `${component?.attributes?.class ?? ''}`.split(/\s+/).includes(className)

const syncGeneratedInquiryFormTexts = (component: Record<string, any>) => {
  if (getComponentType(component) !== 'wb-inquiry-form') return
  const i18nTexts = component.inquiryI18nTexts
  if (!i18nTexts || typeof i18nTexts !== 'object') return

  const fields = Array.isArray(i18nTexts.fields) ? i18nTexts.fields : []
  const fieldTextByCode = new Map(
    fields
      .map((field: any) => [`${field?.code ?? ''}`.trim(), field])
      .filter(([code]) => Boolean(code))
  )

  const syncFieldNode = (fieldNode: any) => {
    const children = getProjectChildren(fieldNode)
    const control = children.find((child: any) => {
      const name = `${child?.attributes?.name ?? ''}`.trim()
      return name && fieldTextByCode.has(name)
    })
    if (!control) return
    const fieldText = fieldTextByCode.get(`${control.attributes?.name ?? ''}`.trim()) as any
    if (!fieldText || typeof fieldText !== 'object') return

    const label = children.find((child: any) => hasClassName(child, 'wb-inquiry__label'))
    ...
    if (control.attributes && typeof fieldText.placeholder === 'string') {
      control.attributes = {
        ...control.attributes,
        placeholder:
          `${control.attributes['data-field-kind'] ?? ''}`.trim() && fieldText.optionPlaceholder
            ? fieldText.optionPlaceholder
            : fieldText.placeholder
      }
    }

    const hint = children.find((child: any) => hasClassName(child, 'wb-inquiry__field-hint'))
  ...
  const walk = (node: any) => {
    if (!node || typeof node !== 'object') return
    if (hasClassName(node, 'wb-inquiry__field')) syncFieldNode(node)
...
const findSavedEntryForSource = (
  source: WebBuilderI18nEntry,
  savedMap: Map<string, WebBuilderI18nEntry>
): WebBuilderI18nEntry | undefined => {
  ...
  const aliases =
    field === 'text' || field === 'html'
      ? [CANONICAL_TEXT_COMPONENT_FIELD]
      : field === CANONICAL_TEXT_COMPONENT_FIELD
        ? (['text', 'html'] as WebBuilderI18nField[])
        : []

  const candidates = aliases
    .map((alias) => savedMap.get(`${source.key}::${alias}`))
    .filter(Boolean) as WebBuilderI18nEntry[]
  if (!candidates.length) return undefined

  return (
    candidates.find((candidate) => candidate.sourceHash === source.sourceHash) ??
    candidates.find(
