...
<template>
  <BlocksProvider v-slot="{ mapCategoryBlocks, append, dragStart, dragStop }">
    <div class="wb-blocks-panel" data-testid="wb-default-blocks-panel">
      <template v-if="mapCategoryBlocks.size">
        <section
          v-for="[category, blocks] in mapCategoryBlocks"
          :key="category"
          class="wb-blocks-panel__group"
        >
          <div class="wb-blocks-panel__category">{{ category || 'Blocks' }}</div>
