...
import { useFontManager, type InstalledFont } from '../composables/useFontManager'
import type { GoogleFontItem } from '../composables/useGoogleFonts'
import { useGlobalTypographyStore } from '@/store/modules/globalTypography'
import WbCtrlFont from './style-controls/WbCtrlFont.vue'
...
const GLOBAL_FONT_PROPERTY = {
  id: 'wb-global-font-family',
...
function handleGlobalFontSelect(payload: { value: string; googleName?: string }) {
  globalTypographyStore.setGlobalFont(payload.value, payload.googleName || '')
}
...
async function handleRemove(font: InstalledFont) {
  try {
    await ElMessageBox.confirm(
      `确定要卸载字体 "${font.family}" 吗？已使用该字体的组件将回退到默认字体。`,
      '卸载字体',
...
function toggleVariant(font: InstalledFont, variant: string) {
  ...
  if (has) {
    ...
    if (newVariants.length === 0) newVariants = ['regular'] // 至少保留一个
  } else {
  ...
  <div class="font-manager">
    ...
    <div class="fm-global">
      <div class="fm-section-title">
        <span>全局字体</span>
        <button
          v-if="globalTypographyStore.fontFamily"
          class="fm-reset-btn"
          type="button"
      ...
      <WbCtrlFont
        :property="GLOBAL_FONT_PROPERTY"
        :model-value="globalTypographyStore.fontFamily"
        :grapes="props.grapes"
      ...
      <div v-for="font in installedFonts" :key="font.family" class="fm-installed-item">
        ...
        <div v-if="expandedFamily === font.family" class="fm-variants">
          ...
          <div class="fm-variant-grid">
            <label
              v-for="v in fontManager.ALL_VARIANTS"
              :key="v"
            ...
            >
              <input
                ...
                @change="toggleVariant(font, v)"
              />
              <span class="fm-variant-text">{{ variantLabel(v) }}</span>
            </label>
          </div>
          <!-- 预览 -->
          <div
