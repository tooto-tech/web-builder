...
import { usePages, useLayoutSettings } from '@/components/WebBuilder/composables'
import {
  cloneLayoutSettings,
  getGrapesPageId,
  getGrapesPageName,
  ...
  type LayoutRule,
  type LayoutSlotKey,
} from '@/components/WebBuilder/utils/layoutSettings'
import {
  loadRulePageOptions,
  type RulePageOption,
} from '@/components/WebBuilder/utils/layoutRulePages'
...
const currentLayoutPage = computed(() => {
  return pagesMgr.pages.find((page: any) => !!getPageLayoutSlot(page)) ?? null
})
...
const activeRules = computed(() => {
  ...
  return layoutSettings.value[slotKey].rules.filter((rule) => layoutTargetMatchesPage(rule.layoutId, layoutPage))
})

async function loadSelectablePages() {
  loadingPages.value = true
  try {
    selectablePages.value = await loadRulePageOptions((params) => getPagePage(params))
  } catch (error) {
...
function updateSettings(mutator: (next: ReturnType<typeof cloneLayoutSettings>) => void) {
  const next = cloneLayoutSettings(layoutSettings.value)
  mutator(next)
  layoutSettings.value = next
  emit('change')
}
