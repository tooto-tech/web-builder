...
import {
  useGlobalColorsStore,
  makeGlobalColorVar,
  parseGlobalColorVar,
} from '@/store/modules/globalColors'
...
watch(() => props.modelValue, (val) => {
  ...
  if (gcId) {
    ...
    const resolved = globalColorsStore.colors.find(c => c.id === gcId)?.value
...
function confirmAddGlobal() {
  ...
  globalColorsStore.addColor(addGlobalName.value.trim(), hexOnly.value)
...
<template>
  <div class="wb-color-picker" @mousedown.stop @pointerdown.stop>
    ...
    <div v-if="linkedGlobalId" class="linked-badge">
      ...
      <span>{{ globalColorsStore.colors.find(c => c.id === linkedGlobalId)?.name ?? '全局颜色' }}</span>
    </div>
    ...
    <div v-if="!hideGlobalColors" class="global-section">
      ...
      <div v-if="globalColorsStore.colors.length > 0" class="global-swatches">
        <div
          v-for="gc in globalColorsStore.colors"
          :key="gc.id"
          class="global-swatch"
