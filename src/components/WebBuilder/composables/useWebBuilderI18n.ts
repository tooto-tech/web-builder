...
import {
  ...
  saveBundle,
  translateEntries,
  type WebBuilderI18nBundleResp,
  type WebBuilderI18nEntry,
  type WebBuilderI18nReviewStatus,
  type WebBuilderI18nEntryStatus,
  type WebBuilderI18nTranslationOrigin
} from '@/api/content/webbuilderI18n'
import * as LanguageApi from '@/api/system/language'
import { hasPageResourceLocator, type PageResourceIdentity } from '@/api/content/page'
import {
  WB_I18N_KEY_ATTR,
...
const normalizeLanguageValue = (language: Pick<LanguageApi.LanguageVO, 'code' | 'slug'>) =>
  `${language.code || language.slug || ''}`.trim()

const languageCodeLabel = (language: Pick<LanguageApi.LanguageVO, 'code' | 'slug'>) =>
  `${language.code || language.slug || ''}`.trim()
...
const toI18nLanguage = (language: LanguageApi.LanguageVO): WebBuilderI18nLanguage | null => {
...
export default function useWebBuilderI18n(options: UseWebBuilderI18nOptions) {
  ...
  const provider = ref('')
  const languages = ref<WebBuilderI18nLanguage[]>(options.languages || [])
  ...
  const loadLanguages = async () => {
    if (options.languages?.length) {
      applyLanguageSelection(options.languages)
      return
    }
    try {
      const list = await LanguageApi.getEnabledLanguageList()
      const enabledLanguages = (Array.isArray(list) ? list : [])
        .sort((left, right) => {
          const defaultSort = (right.defaultLang || 0) - (left.defaultLang || 0)
  ...
  const runAutoTranslateMissing = async (autoOptions: AutoTranslateOptions = {}) => {
    ...
    try {
      const response = await autoTranslateEntries({
        ...resource.value,
  ...
  const confirmContinueAfterI18nIssue = async (action: BlockingProcessAction, reason: string) => {
    ...
    try {
      await ElMessageBox.confirm(
        `多语言自动翻译未全部完成：${reason}。继续${actionText}后，缺失或异常片段可能显示源语言内容或旧译文。是否继续${actionText}？`,
        `多语言${actionText}提醒`,
        {
      ...
      ElMessage.warning(`已${cancelText}，当前编辑内容仍保留在页面中`)
      return false
    }
  }
  ...
  const ensureReadyOrConfirm = async (action: BlockingProcessAction) => {
    ...
    if (!flushed) {
      options.blockingProcess?.stop()
      ElMessage.error(lastError.value || '多语言手工译文保存失败，已中止当前操作')
      return false
    }

    const ready = await autoTranslateMissing({
      publishReady: action === 'publish',
      reloadCurrentLocale: action === 'publish',
      silent: false
    })
