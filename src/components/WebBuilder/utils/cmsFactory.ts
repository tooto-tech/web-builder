...
import { PRODUCT_CARD_CSS } from '../components/registries/dynamic/cms/product/styles'
...
import { composePlaceholderUrl } from './dynamicRenderPipeline'

// ── Loadmore API 端点映射 ──────────────────────────────────────────
const CMS_APP_API_MAP: Record<string, { api: string; entity: string }> = {
  'post-list': { api: '/app-api/content/post/page', entity: 'post' },
  'cases-list': { api: '/app-api/content/post/page', entity: 'post' },
  'media-list': { api: '/app-api/content/media-resource/page', entity: 'media' },
  'product-list': { api: '/app-api/product/spu/page', entity: 'product' }
}
...
(function(){
  ...
  function fetchTagList(useTypeId) {
    var path = useTypeId === 'all' ? '/app-api/content/post-tag/list-all' : '/app-api/content/post-tag/list';
  ...
  function fetchPosts(pageNo) {
    ...
    fetch('/app-api/content/post/page?' + params.toString(), { headers: buildHeaders() })
      .then(function(res){ return res.json(); })
      .then(function(json){
  ...
  function buildHeaders() {
    var headers = {};
    if (cfg.tenantId) headers['tenant-id'] = String(cfg.tenantId);
    return headers;
  }
...
(function(){
  ...
  function fetchPage(page) {
    ...
    var lang = resolveLanguage();
    if (lang && cfg.api.indexOf('/app-api/content/') === 0) {
      params.set('language', lang);
    }

    fetch(cfg.api + '?' + params.toString(), { headers: buildHeaders() })
      .then(function(res){ return res.json(); })
      .then(function(json){
        var data = json && json.data;
        var list = Array.isArray(data && data.list) ? data.list : [];
 ...
 */
function buildSearchIIFE(tenantId: string): string {
  return `
(function(){
  var PAGE_SIZE = 20;
  var currentPage = 1;
  ...
  function doSearch(pageNo, options) {
    ...
    fetch('/app-api/content/search?' + qs, { headers: { 'tenant-id': TENANT_ID } })
    ...
    ({ id, dataCmsComponent, instanceId, isLoopGrid, publishHtml, attributes, props }) => {
      ...
      if (dataCmsComponent === 'product-list' || dataCmsComponent === 'search') {
        ...
        if (tenantId !== undefined && tenantId !== null && tenantId !== '') {
          el.setAttribute('data-tenant-id', String(tenantId))
        }
        ...
        if (!enableTagFilter) {
          ...
          if (templateCard) {
            ...
            iifeScript.textContent = buildPostListTagFilterIIFE({
              ...
              tenantId: String(getEffectiveTenantId() || ''),
              selectionMode: String(
                mergedAttributes['data-tag-selection-mode'] ||
      ...
      if (
        ...
        if (queryTemplateCard) {
          ...
          iifeScript.textContent = buildCategoryQueryOverrideIIFE({
            api:
              dataCmsComponent === 'media-list'
                ? '/app-api/content/media-resource/page'
                : '/app-api/content/post/page',
            paramKey: 'categoryId',
      ...
      if (dataCmsComponent === 'search') {
        const tenantId = getEffectiveTenantId() || ''
        const iifeScript = doc.createElement('script')
        iifeScript.textContent = buildSearchIIFE(tenantId)
        el.appendChild(iifeScript)
      }
      ...
      if (paginationMode === 'none') {
        ...
        const apiInfo = shouldUseLegacyLoadMore ? CMS_APP_API_MAP[dataCmsComponent] : undefined
        if (apiInfo) {
...
(function(){
  ...
  btn.onclick = function() {
    ...
    fetch(cfg.api + '?' + qs, { headers: { 'tenant-id': cfg.tenantId } })
      .then(function(r){ return r.json(); })
      .then(function(json){
